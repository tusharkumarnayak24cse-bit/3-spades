...
const i18n = {
  en:{
    ...
    heroTitle:"Play Kaali Ni Tidi with your friends.",heroText:"Create a private room, share the code, and play with 4, 6, or 8 players. 8-player mode uses two full decks (104 cards). Empty seats can be filled by bots.",
  ...
  gu:{
    ...
    heroTitle:"મિત્રો સાથે કાળી ની તીડી રમો.",heroText:"પ્રાઇવેટ રૂમ બનાવો, કોડ શેર કરો અને 4, 6 અથવા 8 ખેલાડીઓ સાથે રમો. 8 ખેલાડી મોડમાં 2 સંપૂર્ણ ડેક (104 પત્તા) વપરાય છે. ખાલી સીટ બોટ ભરી શકે છે.",
...
function createCard(card,opts={}){
  ...
  const copyText=state?.playerCount===8?`D${card.copy}`:(card.copy>1?"II":"");
...
function render(){
  ...
  $("trickNo").textContent=state.trickNumber;
  renderLobby();
...
function renderHand(){
  ...
  $("legalHint").textContent=state.phase==="playing"&&state.turnIndex===state.viewerIndex?(state.leadSuit?`${suitSymbol[state.leadSuit]} ${language==="gu"?"ફોલો કરો":"follow suit"}`:(language==="gu"?"તમારી લીડ":"Your lead")):"";
}
...
function renderActions(){
  ...
  if(state.phase==="bidding"){
    ...
    const min=state.bid.current==null?state.bid.min:state.bid.current+5;
    panel.innerHTML=`<div class="action-row"><label>${language==="gu"?"બિડ રકમ":"Bid amount"}<input id="bidAmount" type="number" min="${min}" max="${state.bid.max}" step="5" value="${Math.min(min,state.bid.max)}"></label><button id="bidBtn" class="primary" type="button">${language==="gu"?"બિડ":"Bid"}</button><button id="passBtn" type="button">${language==="gu"?"પાસ":"Pass"}</button></div><p class="note">${state.bid.min}–${state.bid.max}, +5</p>`;
    $("bidBtn").addEventListener("click",()=>{beep("bid");socket.emit("bid",{amount:Number($("bidAmount").value),pass:false});});
    $("passBtn").addEventListener("click",()=>socket.emit("bid",{pass:true}));return;
  }

  if(state.phase==="contract"){
    if(state.bid.bidderIndex!==state.viewerIndex){panel.innerHTML=`<strong>${escapeHtml(state.players[state.bid.bidderIndex].name)} ${language==="gu"?"હુકમ અને ગુપ્ત પાર્ટનર પસંદ કરી રહ્યા છે":"is choosing Hukum and hidden partners…"} </strong>`;return;}
    const copyPicker=i=>state.playerCount===8?`<label>${language==="gu"?"ડેક":"Deck"}<select id="partnerCopy${i}"><option value="1">Deck 1</option><option value="2">Deck 2</option></select></label>`:"";
    const partnerHTML=Array.from({length:state.partnerCount},(_,i)=>`<div class="action-row"><label>${language==="gu"?"પાર્ટનર":"Partner"} ${i+1} suit<select id="partnerSuit${i}">${["S","H","D","C"].map(s=>`<option value="${s}">${suitSymbol[s]} ${suitName[s]}</option>`).join("")}</select></label><label>${language==="gu"?"પાર્ટનર":"Partner"} ${i+1} rank<select id="partnerRank${i}">${["A","K","Q","J","10","9","8","7","6","5","4","3","2"].map(r=>`<option value="${r}">${r}</option>`).join("")}</select></label>${copyPicker(i)}</div>`).join("");
    panel.innerHTML=`<div class="contract-grid"><label>${language==="gu"?"હુકમ":"Hukum / Trump"}<select id="trumpPick">${["S","H","D","C"].map(s=>`<option value="${s}">${suitSymbol[s]} ${suitName[s]}</option>`).join("")}</select></label><div class="partner-picks">${partnerHTML}</div><button id="lockContract" class="primary" type="button">${language==="gu"?"ગેમ શરૂ":"Start play"}</button></div><p class="note">${state.playerCount===8?(language==="gu"?"8 ખેલાડી મોડમાં ચોક્કસ Deck 1 / Deck 2 નો પત્તો પસંદ કરો. તે પત્તો રમાશે ત્યારે પાર્ટનર જાહેર થશે.":"In 8-player mode choose the exact Deck 1 / Deck 2 card. Its owner is revealed when that physical card is played."):(language==="gu"?"જે પત્તા તમારી પાસે નથી તે પસંદ કરો. પત્તો ખુલશે ત્યારે પાર્ટનર જાહેર થશે.":"Choose cards you do not hold. Their owners are revealed as partners when those cards are played.")}</p>`;
    $("lockContract").addEventListener("click",()=>{
      const partnerCards=[];for(let i=0;i<state.partnerCount;i++)partnerCards.push({suit:$(`partnerSuit${i}`).value,rank:$(`partnerRank${i}`).value,copy:state.playerCount===8?Number($(`partnerCopy${i}`).value):1});
      beep("win");socket.emit("contract",{trump:$("trumpPick").value,partnerCards});
    });return;
  }

  if(state.phase==="playing"){
    panel.innerHTML=state.turnIndex===state.viewerIndex?`<strong class="success">${language==="gu"?"તમારી ચાલ — હાઇલાઇટ થયેલો પત્તો રમો.":"Your turn — play a highlighted card."}</strong>`:`<strong>${escapeHtml(state.players[state.turnIndex].name)} ${language==="gu"?"ની ચાલ":"to play…"} </strong>`;return;
  }
  ...
  if(state.phase==="roundEnd"){
    ...
    panel.innerHTML=`<div class="section-title"><div><h3 class="${made?"success":"danger"}">${made?(language==="gu"?"કોન્ટ્રાક્ટ સફળ":"Contract made"):(language==="gu"?"કોન્ટ્રાક્ટ નિષ્ફળ":"Contract failed")}</h3><p>${points}/${state.bid.current}</p></div>${state.host?`<button id="nextRoundBtn" class="primary" type="button">${language==="gu"?"આગલો રાઉન્ડ":"Deal next round"}</button>`:""}</div>`;
    if(state.host)$("nextRoundBtn").addEventListener("click",()=>socket.emit("nextRound"));
  }
}

function renderScoreboard(){
  $("scoreboard").innerHTML=`<div class="score-row"><strong>${language==="gu"?"ખેલાડી":"Player"}</strong><span>Total</span><span>${language==="gu"?"રાઉન્ડ":"Round"}</span><span>Team</span></div>${state.players.map(p=>`<div class="score-row"><strong>${escapeHtml(p.name)}</strong><span>${p.score}</span><span>${p.roundPoints}</span><span>${p.team||"?"}</span></div>`).join("")}`;
}
