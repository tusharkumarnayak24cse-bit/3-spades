...
function renderActions(){
  ...
  if(state.phase==="playing"){
    ...
    const turnLine=state.turnIndex===state.viewerIndex?`<div class="turn-banner mine"><span>●</span><strong>${language==="gu"?"તમારી ચાલ":"YOUR TURN"}</strong><small>${language==="gu"?"હાઇલાઇટ થયેલો પત્તો રમો":"Play a highlighted card"}</small></div>`:`<div class="turn-banner"><span>●</span><strong>${escapeHtml(state.players[state.turnIndex].name)}</strong><small>${language==="gu"?"ની ચાલ":"is playing"}</small></div>`;
