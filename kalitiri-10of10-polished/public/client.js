...
let turnClockKey = "";
let turnClockStartedAt = Date.now();
...
function beep(kind="tap"){
  ...
  try{
    ...
    const patterns={
      ...
      win:[[660,.04,.09],[820,.045,.1],[990,.04,.14]], chat:[[520,.025,.07]], turn:[[760,.03,.08],[940,.025,.09]], trick:[[520,.035,.07],[680,.03,.07],[820,.035,.11]]
    };
...
function renderSeats(){
  ...
  state.players.forEach((p,i)=>{
    ...
    const role=p.team==="bidder"?(language==="gu"?"બિડર ટીમ":"Bidder"):p.team==="defense"?(language==="gu"?"ડિફેન્સ":"Defense"):"";
    const isTurn=state.turnIndex===i&&state.phase==="playing";
    seat.innerHTML=`<div class="seat-card">${isTurn?`<div class="turn-chip">${language==="gu"?"ચાલ":"TURN"}</div><div class="seat-timer" aria-hidden="true"><i></i></div>`:""}<div class="avatar">${p.avatar||"😎"}</div><span class="name">${escapeHtml(p.name)}${i===state.viewerIndex?" · You":""}</span><div class="seat-stats"><span class="card-count">🂠 ${p.cards}</span>${role?`<span class="role-badge ${p.team}">${role}</span>`:""}</div>${p.voiceJoined&&i!==state.viewerIndex?`<button type="button" class="voice-person-btn">${individuallyMuted.has(i)?"🔇 Unmute":"🔊 Mute"}</button>`:""}</div>`;
...
function renderTrick(){
  ...
  if(state.lastTrick && !state.trick.length && state.phase==="playing"){
    ...
    flash.innerHTML=`<span>🏆</span><strong>${escapeHtml(winner?.name||"Player")}</strong><small>+${state.lastTrick.points||0} pts</small>`;
...
function renderActions(){
  ...
  if(state.phase==="bidding"){
    ...
    panel.innerHTML=`<div class="bid-head"><div><span class="eyebrow">${language==="gu"?"હાલની સૌથી મોટી બિડ":"HIGHEST BID"}</span><div class="big-bid">${state.bid.current??"—"}</div></div><strong class="your-turn-callout">${language==="gu"?"તમારી બિડ":"Your bid"}</strong></div><div class="quick-bids">${quickBids.map(v=>`<button class="quick-bid" type="button" data-bid="${v}" ${canBid?"":"disabled"}>${v}</button>`).join("")}</div><div class="action-row bid-custom"><label>${language==="gu"?"કસ્ટમ બિડ":"Custom bid"}<input id="bidAmount" type="number" min="${min}" max="${state.bid.max}" step="${inc}" value="${Math.min(min,state.bid.max)}" ${canBid?"":"disabled"}></label><button id="bidBtn" class="primary" type="button" ${canBid?"":"disabled"}>${language==="gu"?"બિડ":"Bid"}</button><button id="passBtn" class="pass-btn" type="button">${language==="gu"?"પાસ":"Pass"}</button></div><p class="note">${state.bid.min}–${state.bid.max}, +${inc} · ${language==="gu"?"સૌથી મોટી બિડ કરનાર પ્રથમ હાથ લીડ કરશે":"highest bidder leads the first trick"}${!canBid?` · ${language==="gu"?"મહત્તમ બિડ થઈ ગઈ — પાસ કરો":"maximum bid reached — pass"}`:""}</p>`;
...
socket.on("state",next=>{
  const prev=state;
  const nextTurnKey=`${next.phase}:${next.round}:${next.trickNumber}:${next.turnIndex}`;
  if(nextTurnKey!==turnClockKey){turnClockKey=nextTurnKey;turnClockStartedAt=Date.now();}
...
socket.on("connect",()=>{setNetStatus("online","● Online");});
...
socket.io.on("reconnect",()=>{setNetStatus("online","● Online");toast(language==="gu"?"ફરી કનેક્ટ થયું.":"Reconnected.");});
socket.on("connect_error",()=>setNetStatus("offline","● Offline"));

setInterval(()=>{
  if(!state||state.phase!=="playing")return;
  const elapsed=Math.floor((Date.now()-turnClockStartedAt)/1000),remaining=Math.max(0,30-elapsed);
  const c=$("turnCountdown");if(c)c.textContent=String(remaining);
  document.querySelectorAll(".seat.active .seat-timer i").forEach(el=>el.style.transform=`scaleX(${remaining/30})`);
},250);
