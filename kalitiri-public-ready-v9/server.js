...
const TURN_TIMEOUT_MS = Math.max(10_000, Number(process.env.TURN_TIMEOUT_MS || 30_000));
...
app.use((req, res, next) => {
  ...
  res.setHeader("Content-Security-Policy", "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self' https: wss:; media-src 'self' blob:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'");
  next();
});
