...
const GAME_SERVER = (location.protocol === "file:" || location.protocol === "capacitor:") ? PACKAGED_GAME_SERVER : null;
...
function renderHand(){
  ...
  state.hand.forEach((card,cardIndex)=>{
    ...
    if(can){
      ...
      const play=()=>{beep("deal");socket.emit("playCard",{cardId:card.id},res=>{if(res&&!res.ok)toast(res.error);});};
...
function renderActions(){
  ...
  if(state.phase==="bidding"){
    if(state.bid.turnIndex!==state.viewerIndex){panel.innerHTML=`<strong>${escapeHtml(state.players[state.bid.turnIndex].name)} ${language==="gu"?"ની બિડની રાહ":"is bidding…"} <b id="actionCountdown" class="inline-countdown">30</b></strong>`;return;}
    ...
    panel.innerHTML=`<div class="bid-head"><div><span class="eyebrow">${language==="gu"?"હાલની સૌથી મોટી બિડ":"HIGHEST BID"}</span><div class="big-bid">${state.bid.current??"—"}</div></div><strong class="your-turn-callout">${language==="gu"?"તમારી બિડ":"Your bid"} <b id="actionCountdown" class="inline-countdown">30</b></strong></div><div class="quick-bids">${quickBids.map(v=>`<button class="quick-bid" type="button" data-bid="${v}" ${canBid?"":"disabled"}>${v}</button>`).join("")}</div><div class="action-row bid-custom"><label>${language==="gu"?"કસ્ટમ બિડ":"Custom bid"}<input id="bidAmount" type="number" min="${min}" max="${state.bid.max}" step="${inc}" value="${Math.min(min,state.bid.max)}" ${canBid?"":"disabled"}></label><button id="bidBtn" class="primary" type="button" ${canBid?"":"disabled"}>${language==="gu"?"બિડ":"Bid"}</button><button id="passBtn" class="pass-btn" type="button">${language==="gu"?"પાસ":"Pass"}</button></div><p class="note">${state.bid.min}–${state.bid.max}, +${inc} · ${language==="gu"?"સૌથી મોટી બિડ કરનાર પ્રથમ હાથ લીડ કરશે":"highest bidder leads the first trick"}${!canBid?` · ${language==="gu"?"મહત્તમ બિડ થઈ ગઈ — પાસ કરો":"maximum bid reached — pass"}`:""}</p>`;
...
setInterval(()=>{
  ...
  const timeout=Math.max(1000,state.actionTimeoutMs||30000);
