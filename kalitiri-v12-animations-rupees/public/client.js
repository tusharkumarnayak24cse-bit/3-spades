...
// Virtual game rupees: local practice/reward currency only. No cash value or payments.
const WALLET_KEY = "knt_virtual_rupees_v1";
const REWARDED_ROUNDS_KEY = "knt_rewarded_rounds_v1";
const STARTING_RUPEES = 1000;
let virtualRupees = Math.max(0, Number(localStorage.getItem(WALLET_KEY) || STARTING_RUPEES));
let lastRoundReward = 0;
function formatRupees(v){ return Number(v||0).toLocaleString("en-IN"); }
function updateWalletUI(){
  const el=$("walletBalance"); if(el) el.textContent=formatRupees(virtualRupees);
}
function readRewardedRounds(){ try{return JSON.parse(localStorage.getItem(REWARDED_ROUNDS_KEY)||"[]");}catch{return [];} }
function rewardRoundIfNeeded(next){
  if(!next || next.phase!=="roundEnd" || next.viewerIndex==null) return 0;
  const key=`${next.code}:${next.round}`;
  const rewarded=readRewardedRounds();
  if(rewarded.includes(key)) return 0;
  const bidderPoints=(next.players||[]).filter((_,i)=>(next.bidderTeam||[]).includes(i)).reduce((sum,p)=>sum+(p.roundPoints||0),0);
  const made=bidderPoints>(next.bid?.current ?? Infinity);
  const viewerTeam=next.players?.[next.viewerIndex]?.team;
  const viewerWon=(viewerTeam==="bidder"&&made)||(viewerTeam==="defense"&&!made);
  const reward=viewerWon?100:25;
  virtualRupees+=reward; lastRoundReward=reward;
  localStorage.setItem(WALLET_KEY,String(virtualRupees));
  rewarded.push(key); localStorage.setItem(REWARDED_ROUNDS_KEY,JSON.stringify(rewarded.slice(-80)));
  updateWalletUI();
  setTimeout(()=>animateRupeeBurst(reward,viewerWon),120);
  return reward;
}
function animateRupeeBurst(amount,big=false){
  const layer=$("celebrationLayer"); if(!layer)return; layer.innerHTML=""; layer.classList.add("show");
  const banner=document.createElement("div"); banner.className=`rupee-reward-banner ${big?"big":""}`;
  banner.innerHTML=`<span>₹</span><div><small>${language==="gu"?"ગેમ રિવોર્ડ":"GAME REWARD"}</small><strong>+₹${formatRupees(amount)}</strong></div>`; layer.appendChild(banner);
  const symbols=["₹","♠","♥","♦","♣","₹","₹"];
  for(let i=0;i<(big?24:14);i++){const c=document.createElement("i");c.className="reward-particle";c.textContent=symbols[i%symbols.length];c.style.setProperty("--x",`${Math.round((Math.random()-.5)*86)}vw`);c.style.setProperty("--r",`${Math.round((Math.random()-.5)*500)}deg`);c.style.setProperty("--delay",`${(Math.random()*.22).toFixed(2)}s`);layer.appendChild(c);}
  beep(big?"win":"trick"); setTimeout(()=>{layer.classList.remove("show");layer.innerHTML="";},1800);
}
...
function renderActions(){
  ...
  if(state.phase==="roundEnd"){
    ...
    const viewerTeam=state.players[state.viewerIndex]?.team;
    const viewerWon=(viewerTeam==="bidder"&&made)||(viewerTeam==="defense"&&!made);
    const rewardShown=lastRoundReward || (viewerWon?100:25);
    panel.innerHTML=`<div class="section-title round-result"><div><h3 class="${made?"success":"danger"}">${made?(language==="gu"?"કોન્ટ્રાક્ટ સફળ":"Contract made"):(language==="gu"?"કોન્ટ્રાક્ટ નિષ્ફળ":"Contract failed")}</h3><p>${points} ${language==="gu"?"પોઇન્ટ · બિડ":"points · bid"} ${state.bid.current} (${language==="gu"?"બિડ કરતાં વધારે જરૂરી":"must be higher than bid"})</p><div class="round-rupee-reward"><span>₹</span><b>+₹${formatRupees(rewardShown)}</b><small>${viewerWon?(language==="gu"?"વિન બોનસ":"win reward"):(language==="gu"?"પ્લે બોનસ":"play reward")} · ${language==="gu"?"વર્ચ્યુઅલ":"virtual"}</small></div></div>${state.host?`<button id="nextRoundBtn" class="primary" type="button">${language==="gu"?"આગલો રાઉન્ડ":"Deal next round"}</button>`:""}</div>`;
...
socket.on("state",next=>{
  const prev=state;
  if(prev && next.round!==prev.round) lastRoundReward=0;
  if(next.phase==="roundEnd" && (!prev || prev.phase!=="roundEnd")) rewardRoundIfNeeded(next);
